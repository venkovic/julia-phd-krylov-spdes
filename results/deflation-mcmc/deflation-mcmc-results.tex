...
\end{align}
After elimination of $\mathbf{u}_{\mathrm{I}}$ from $\mathbf{A}\mathbf{u}=\mathbf{b}$, we have
\begin{align}
\mathbf{S}\mathbf{u}_{\Gamma}=\mathbf{b}_S
\end{align}
where $\mathbf{b}_S=$ and the Schur complement, $\mathbf{S}$, is applied 
\begin{align}
\mathbf{S}\mathbf{u}_\Gamma=\sum_{d=1}^{n_d}\mathbf{S}_d\mathbf{u}_{\Gamma,d}
\end{align}
with
\begin{align}
\mathbf{S}_d\mathbf{u}_{\Gamma,d}=
\mathbf{A}_{\Gamma\Gamma,d}\mathbf{u}_{\Gamma,d}-
\mathbf{A}_{\Gamma\mathrm{I},d}\mathbf{A}_{\mathrm{II},d}^{-1}\mathbf{A}_{\mathrm{I}\Gamma,d}\mathbf{u}_{\Gamma,d}.
\end{align}
For non-overlapping partitions, $\mathbf{A}_{\mathrm{II},d}$ is SPD but $\mathbf{S}_{d}$ is semi-definite.

\subsubsection{Neumann-Neumann (NN)}
The NN preconditioner $\mathbf{M}_{NN}^{-1}$, see~\cite{}, is defined as follows for Eq.~\eqref{},
\begin{align}
\mathbf{M}_{NN}^{-1}\mathbf{u}_\Gamma=\sum_{d=1}^{n_d}\boldsymbol{\mathcal{R}}_{\Gamma,d}
\end{align}
