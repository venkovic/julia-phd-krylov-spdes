...
%\makeatother
\usepackage{tikz}
\renewcommand{\algorithmiccomment}[1]{{\color{gray}$\triangleright$ #1}}


% My math operators
\input{/home/venkovic/Dropbox/Git/tex-thesis-ubx-cerfacs/template_ub/my-operators.tex}
...
\begin{algorithm}[ht]
	...
	\begin{algorithmic}[1]
		...
		\ENSURE{$\invMNN\mathbf{r}$}
		\FOR{$d=1,\dots,n_d$}
		\STATE{$\mathbf{r}^{(d)}:=\mathbf{D}_d^T\RGloc{d}^T\mathbf{r}$}
		\STATE{Solve for any $\mathbf{z}^{(d)}$ s.t. $\mathbf{S}^{(d)}\mathbf{z}^{(d)}=\mathbf{r}^{(d)}$}
		\ENDFOR
        \STATE{$\mathbf{z}:=\sum_{d=1}^{n_d}\RGloc{d}\mathbf{D}_d\mathbf{r}^{(d)}$}
		\RETURN{$\mathbf{z}$}
	\end{algorithmic}
\end{algorithm}
