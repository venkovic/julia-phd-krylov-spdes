...
"""
     pll_do_global_mass_covariance_reduced_assembly(cells::Array{Int,2},
...
Input:
 ...
 `domain::Union{Dict{Int,SubDomain}`,
         `Array{SubDomain,1}}`
  Collection of all SubDomain data structures.
         
 `idom::Int`,
  subdomain index.
  
 `md::Array{Int,1}`, Numbers of local eigenmodes for each subdomain.
        ...
        for α in 1:md[idom]
          ...
          for β in 1:md[jdom]
            ...
            for j in 1:nnode_jdom
              Φ_j_β = domain[jdom].Φ[j, β]

              for i in 1:nnode_idom
                K[ind_α_idom, ind_β_jdom] += domain[idom].Φ[i, α] * C[i, j] * Φ_j_β
              end
            end
          end # end β
        end # end α
...
"""
     pll_solve_local_kl(cells::Array{Int,2},
                       points::Array{Float64,2},
                       epart::Array{Int,1},
                       cov::Function,
                       nev::Int,
...
function pll_solve_local_kl(cells::Array{Int,2},
  ...
  #λ, Φ  = LinearAlgebra.eigen(C, Array(M), sortby=λ->-λ)
